...
function DirectionCard({ stat }: { stat: DirectionStat }) {
  ...
  return (
    <div className={cx("flex-1 min-w-[140px] rounded-xl border p-5 transition-colors duration-200", isLong ? "bg-emerald-400/[0.03] border-emerald-400/[0.1] hover:border-emerald-400/20" : "bg-red-400/[0.03] border-red-400/[0.1] hover:border-red-400/20")}>
      <div className="flex items-center gap-2 mb-4">
        <span className={cx("font-mono text-[11px] tracking-[0.08em]", isLong ? "text-emerald-400" : "text-red-400")}>{isLong ? "▲" : "▼"}</span>
        <span className={cx("font-mono text-[11px] uppercase tracking-[0.14em]", isLong ? "text-emerald-400/70" : "text-red-400/70")}>{stat.direction}</span>
      ...
      <div className="mt-4 h-[3px] rounded-full bg-white/[0.05] overflow-hidden">
        <div className={cx("h-full rounded-full transition-all duration-700", isLong ? "bg-emerald-400/50" : "bg-red-400/50")} style={{ width: `${stat.winRate}%` }} />
      </div>
    </div>
  );
}
      ...
      <div className="p-5">
        {directions.every(d => d.trades === 0) ? (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {directions.map(stat => <DirectionCard key={stat.direction} stat={stat} />)}
          </div>
        )}
      </div>
    </div>
