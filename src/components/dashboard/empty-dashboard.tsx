...
export function EmptyDashboard() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[440px] text-center px-8 py-14 animate-fade-up">

      {/* Chart illustration */}
      <div className="mb-8">
        <svg width="72" height="52" viewBox="0 0 72 52" fill="none" className="opacity-30">
          <path d="M4 46 L16 30 L28 36 L42 16 L56 24 L68 8" stroke="#f59e0b" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          <circle cx="68" cy="8" r="3" fill="#f59e0b" />
          <line x1="4" y1="50" x2="68" y2="50" stroke="rgba(255,255,255,0.1)" strokeWidth="1"/>
        </svg>
      </div>

      {/* Headline */}
      <h2 className="font-mono font-normal text-[20px] tracking-[-0.025em] text-white mb-3 leading-snug">
        Your edge is waiting<br/>
        <span className="text-white/35">to be found.</span>
      </h2>

      <p className="font-mono text-[12px] leading-[1.75] text-white/35 max-w-[360px] mb-8">
      ...
      <div className="w-full max-w-[320px] mb-8 text-left">
        ...
        <div className="flex flex-col gap-[6px]">
          {INSIGHT_ITEMS.map(({ icon, text }) => (
            <div key={text} className="flex items-center gap-3 px-3 py-[9px] rounded-lg bg-white/[0.02] border border-white/[0.04]">
              <span className="text-amber-400/40 text-sm leading-none shrink-0">{icon}</span>
      ...
      <Link
        href="/trades/new"
        className="inline-flex items-center gap-2 px-6 py-[12px] bg-amber-400 text-[#07090d] rounded-lg font-mono text-[12px] font-semibold tracking-[0.06em] uppercase no-underline hover:bg-amber-300 transition-colors duration-150"
      >
