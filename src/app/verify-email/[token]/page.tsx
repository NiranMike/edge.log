...
import { VerifySuccessClient }   from "@/components/auth/verify-success-client";
...
export default async function VerifyEmailTokenPage({ params }: Props) {
  ...
  const result = await emailVerifyService.verify(token);

  if (result.ok) {
    // Render a Client Component that invokes the Server Action.
    // Server Actions called from Client Components have cookie write access;
    // calling them directly from a Server Component does not.
    return <VerifySuccessClient />;
  }

  // Verification failed — show error with a resend option
  return (
